...
- 공지 상세 보기: 본문 내용, AI 3줄 요약, 원문 링크(인앱 웹뷰), 마감 D-Day 확인
- 스크랩 / 공유: 공지 스크랩 토글, 안드로이드 공유 인텐트로 공유하기
...
- 선택 날짜가 오늘이면 "오늘 일정"으로 표시

### ⚙️ 5. 마이페이지 및 맞춤형 알림 설정
- 내 프로필 조회: 등록한 대학교, 학과, 학년 정보 확인 / 학교 정보 수정
- 스크랩 / 스펙 로그(등록·상세·추가) / 관심 키워드 관리
- 세부 알림 토글 제어: 카테고리별 알림 On/Off
- 알림(공지사항) 목록 확인, 문의하기(고객센터)
...
## 📂 프로젝트 폴더 구조
    ...
    │   ├── search/          # 검색 화면
    │   ├── calendar/        # 캘린더 화면
    │   ├── mypage/           # 마이페이지, 스크랩, 스펙로그, 알림설정, 학교정보수정, 문의하기
    │   ├── notification/     # 알림(공지) 목록
...
|-----------|-----------|----------|------|
| 로그인 | LoginScreen | 앱 최초 실행 | ✅ 완료 (카카오/구글 SDK 실연동) |
| 온보딩 - 대학 선택 | OnboardingUniversityScreen | 로그인 완료 후 최초 1회 | ✅ 완료 (실 API 연동) |
| 온보딩 - 프로필 설정 | OnboardingProfileScreen | 대학 선택 후 | ✅ 완료 (실 API 연동) |
| 온보딩 - 관심 키워드 | OnboardingKeywordScreen | 프로필 설정 후 | ✅ 완료 (실 API 연동) |
| 홈 | HomeScreen | 설정 완료 후 / 메인 진입점 | ✅ 완료 (실 API 연동) |
| 공지 상세 | NoticeDetailScreen | 홈 → 아이템 클릭 | ✅ 완료 (실 API 연동, 스크랩/공유/원문웹뷰) |
| 검색 | SearchScreen | 하단 탭 → 검색 | ✅ 완료 (실 API 연동) |
| 캘린더 | CalendarScreen | 하단 탭 → 캘린더 | ✅ 완료 (실 API 연동) |
| 마이페이지 | MyPageScreen | 하단 탭 → 마이페이지 | ✅ 완료 (실 API 연동) |
| 스크랩 | ScrapScreen | 마이페이지 → 스크랩 | ✅ 완료 |
| 관심 키워드 관리 | KeywordManageScreen | 마이페이지 → 키워드 관리 | ✅ 완료 |
| 스펙 로그 / 상세 / 추가 | SpecLogScreen / SpecDetailScreen / SpecAddScreen | 마이페이지 → 스펙 로그 | ✅ 완료 |
| 학교 정보 수정 | SchoolInfoEditScreen | 마이페이지 → 학교 정보 수정 | ✅ 완료 |
| 알림 설정 | NotificationSettingsScreen | 마이페이지 → 알림 설정 | ✅ 완료 |
| 알림(공지) 목록 | NotificationScreen | 상단바 알림 아이콘 | ✅ 완료 |
| 문의하기 | SupportNoticeScreen | 마이페이지 → 문의하기 | ✅ 완료 |
...
### 네비게이션 플로우
    ...
    앱 실행
     ...
     └─ 재실행 (로그인 유지)
         └─ 홈 (하단 탭: 홈 / 검색 / 캘린더 / 마이)
              ├─ 홈 → 공지 상세 → (원문 웹뷰 / 공유 / 스크랩)
              ├─ 검색 → 공지 상세
              ├─ 캘린더 → 공지 상세
              └─ 마이페이지 → 스크랩 / 스펙 로그 / 키워드 관리 / 학교정보수정 / 알림설정 / 문의하기
